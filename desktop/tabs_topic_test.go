...
func TestV05LegacyEventSessionsImportIntoGlobalTopic(t *testing.T) {
	...
	legacyDir := filepath.Join(home, ".deepseek-orca", "sessions")
...
func TestListProjectTreeShowsEmptyGlobalWhenNoProjects(t *testing.T) {
	...
	if nodes[0].Kind != "global_folder" || nodes[0].Label != "独立工作区" || len(nodes[0].Children) != 0 {
		t.Fatalf("project tree = %#v, want empty independent workspace folder", nodes)
	}
}
