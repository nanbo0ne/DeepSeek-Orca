...
extern void OrcaMarkSystemQuit(void);
...
static NSApplicationTerminateReply orcaApplicationShouldTerminate(id self, SEL _cmd, NSApplication *sender) {
    OrcaMarkSystemQuit();
...
static void orcaWailsContextQuit(id self, SEL _cmd) {
    OrcaMarkSystemQuit();
...
void installOrcaSystemQuitHook(void) {
    ...
    if (method != NULL) {
        IMP replacement = (IMP)orcaApplicationShouldTerminate;
    ...
    if (quitMethod != NULL) {
        IMP replacement = (IMP)orcaWailsContextQuit;
