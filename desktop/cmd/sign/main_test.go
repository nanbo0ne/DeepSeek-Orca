...
func TestSignFiles(t *testing.T) {
	...
	dir := t.TempDir()
	artifact := filepath.Join(dir, "DeepSeek-Orca-linux-amd64.tar.gz")
...
func TestGenManifest(t *testing.T) {
	dir := t.TempDir()
	names := []string{
		"DeepSeek-Orca-darwin-arm64.zip",
		"DeepSeek-Orca-darwin-amd64.zip",
		"DeepSeek-Orca-windows-amd64-installer.exe",
		"DeepSeek-Orca-windows-amd64.zip", // portable download, not the updater channel
	...
	t.Setenv("GITHUB_REPOSITORY", "nanbo0ne/DeepSeek-Orca")
	...
	wantURL := "https://github.com/nanbo0ne/DeepSeek-Orca/releases/download/desktop-v1.2.0/DeepSeek-Orca-windows-amd64-installer.exe"
	if win.URL != wantURL {
	...
	if !strings.HasSuffix(lin.URL, "/DeepSeek-Orca-linux-amd64.tar.gz") {
