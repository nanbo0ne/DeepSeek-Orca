...
func resetTree(repo string) {
	...
	_ = exec.Command("git", "-C", repo, "clean", "-fd", "-e", "orca.toml").Run()
}
...
func renderDiff(r diffReport) string {
	...
	fmt.Fprintf(&b, "## 🤖 O.R.C.A e2e — diff test-gen\n\n")
...
// repoRelFromModulePath turns "github.com/nanbo0ne/O.R.C.A-for-Windows/internal/agent/foo.go" into
...
func repoRelFromModulePath(p string) string {
	...
	prefix := "github.com/nanbo0ne/O.R.C.A-for-Windows/"
