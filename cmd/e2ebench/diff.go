...
func resetTree(repo string) {
	...
	_ = exec.Command("git", "-C", repo, "clean", "-fd", "-e", "deepseek-orca.toml").Run()
}
...
func renderDiff(r diffReport) string {
	...
	fmt.Fprintf(&b, "## 🤖 DeepSeek-Orca e2e — diff test-gen\n\n")
...
// repoRelFromModulePath turns "deepseek-orca/internal/agent/foo.go" into
