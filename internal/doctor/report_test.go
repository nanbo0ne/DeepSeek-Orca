...
func TestCollectReportRedactsSecrets(t *testing.T) {
	t.Setenv("DEEPCODE_TEST_SECRET", "sk-live-secret")
	...
	cfg.Providers = []config.ProviderEntry{{
		...
		Model:     "model-a",
		APIKeyEnv: "DEEPCODE_TEST_SECRET",
	}}
...
func TestCollectReportDoesNotRequireAPIKey(t *testing.T) {
	...
	if !strings.Contains(text, "deepcode 1.2.3 doctor") {
...
func TestRenderTextSurfacesWarningsUpTop(t *testing.T) {
	text := RenderText(Report{Warnings: []string{"config deepcode.toml: parse boom"}})
