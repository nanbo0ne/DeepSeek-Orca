...
var English = Messages{
	...
	SetKeyHint:      "Set your API key, then run `deepcode chat`.",
	ConfigLabel:     "config",
	ModelsLabel:     "models",
	...
	StepScaffold:    "scaffold deepcode.toml",
	StepSetKey:      "set API key",

	InitHint:       "Project memory (AGENTS.md) is generated in-session: run `deepcode chat`, then `/init` — the model analyzes the codebase and writes it. For configuration, use `deepcode setup`.",
	StepSetKeyHint: "run `deepcode setup`, or export DEEPSEEK_API_KEY=…",
	StepChatDesc:   "interactive session",
	StepRunDesc:    "one-shot task",
	HelpFooter:     "deepcode help · all commands",

	ChatTip:           "Context is kept across turns. Type 'exit' or Ctrl-D to quit.",
	TurnCancelled:     "cancelled — back to prompt",
	NoSessionToResume: "no saved session to resume — start a new one with `deepcode chat`",
	...
	OutputStyleHint:    "set agent.output_style in deepcode.toml to apply one (takes effect next session)",
	ThemeHeader:        "themes:",
	...
	SlashMCPNone:                 "no MCP servers configured — add a [[plugins]] entry in deepcode.toml",
	...
	ArgLanguageAuto: "auto-detect from DEEPCODE_LANG / locale",
	...
	ListHooksNone:       "hooks: none active — configure in .deepcode/settings.json (project, after trust) or ~/.deepcode/settings.json (global)",
	ListMcpHeader:       "mcp servers",
	ListMcpNone:         "mcp: no servers connected — add one in deepcode.toml ([[plugins]]) or a project .mcp.json",

	MemoryNone:             "memory: none — add with “/remember <note>” or create DEEPCODE.md in the project root",
	...
	MissingKeyIntro:       "deepcode.toml is ready — just an API key away.",
	WroteFileFmt:          "Wrote %s",
	SetupComplete:         "Setup complete.",
	SetupCancelled:        "setup cancelled.",
	TryHintFmt:            "Try: %s",
	NextHint:              "Next: set your API key (run `deepcode setup` or export DEEPSEEK_API_KEY=...), then run `deepcode run \"your task\"`.",
	...
	SkipStaleCustomEntryFmt:    "skipping stale %q entry from deepcode.toml (pointing at %s) — please remove it from [[providers]]",
	...
	UsageRunHint:              "usage: deepcode run [--model NAME] <task>",
	ErrorPrefix:               "error:",
	...
	ProviderErrAuth:                "Authentication failed (HTTP 401): your API key is missing, wrong, or expired. Check the key in .env or run `deepcode setup`.",
	...
	UsageBody: `deepcode — a config- and plugin-driven coding agent (multi-model)

Usage:
  deepcode chat [--model NAME] [-c|--continue] [--resume]   interactive session (multi-turn; -c resumes the latest, --resume picks one)
  deepcode run  [--model NAME] [--max-steps N] [-c|--continue] [--resume PATH] <task>   run one task and exit
  deepcode serve [--model NAME] [--addr HOST:PORT]      serve the session over HTTP+SSE (browser client at /)
  deepcode acp [--model NAME]                           serve Agent Client Protocol over stdio (also: deepcode --acp)
  deepcode setup [path]                                 interactive config wizard; writes deepcode.toml (+ .env)
  deepcode config auto-plan [off|on]                    configure automatic plan mode
  deepcode mcp <add|remove|list>                        manage MCP servers in deepcode.toml
  deepcode doctor [--json]                              print redacted local diagnostics
  deepcode bot start|doctor|weixin-login                multi-channel IM bot gateway
  deepcode version
  deepcode help

Examples:
  deepcode chat
  deepcode chat --continue
  deepcode run "implement the TODOs in main.go"
  deepcode run --model mimo-pro "add unit tests for this function"
  echo "explain this code" | deepcode run

Configuration:
  Resolution: flag > ./deepcode.toml > ~/.config/deepcode/config.toml > built-in defaults
  Secrets come from the environment via api_key_env (e.g. DEEPSEEK_API_KEY).
  Run 'deepcode setup' to scaffold a config; see docs/SPEC.md.
`,
}
