...
var Chinese = Messages{
	...
	SetKeyHint:      "配置一个供应商 API key 后运行 `orca chat`。",
	ConfigLabel:     "配置",
	ModelsLabel:     "模型",
	...
	Ready:           "已就绪",
	GetStarted:      "开始使用",
	StepScaffold:    "生成 orca.toml",
	StepSetKey:      "设置 API key",

	InitHint:       "项目记忆（ORCA.md）在会话内由模型生成：运行 `orca chat`，然后 `/init` —— 模型会分析代码库并写入。配置请用 `orca setup`。",
	StepSetKeyHint: "运行 `orca setup` 配置供应商 API key。",
	StepChatDesc:   "交互式会话",
	StepRunDesc:    "执行单次任务",
	HelpFooter:     "orca help · 查看全部命令",

	ChatTip:           "对话上下文将跨轮保留。输入 'exit' 或按 Ctrl-D 退出。",
	TurnCancelled:     "已取消 — 回到提示符",
	NoSessionToResume: "没有可恢复的会话 — 用 `orca chat` 开一个新的",
	...
	OutputStyleHint:    "在 orca.toml 设置 agent.output_style 即可启用（下次会话生效）",
	ThemeHeader:        "主题：",
	...
	SlashMCPNone:                 "没有配置 MCP 服务器 — 在 orca.toml 加一个 [[plugins]] 条目",
	...
	ArgEffortMax:    "最高推理",
	ArgThemeCurrent: "当前",
	ArgLanguageAuto: "从 ORCA_LANG / 系统 locale 自动检测",
	...
	ListHooksNone:       "无生效 hooks — 在 .orca/settings.json（项目，需信任后）或 ~/.orca/settings.json（全局）配置",
	ListMcpHeader:       "MCP 服务器",
	ListMcpNone:         "未连接 MCP 服务器 — 在 orca.toml（[[plugins]]）或项目 .mcp.json 中添加",

	MemoryNone:             "还没有加载任何记忆 — 输入 “/remember 内容” 可快速记录，也可以在项目根目录创建 ORCA.md",
	...
	MissingKeyIntro:       "orca.toml 已配置好 — 只差一个供应商 API key 就可以开始。",
	WroteFileFmt:          "已写入 %s",
	SetupComplete:         "设置完成。",
	SetupCancelled:        "设置已取消。",
	TryHintFmt:            "试试: %s",
	NextHint:              "下一步：运行 `orca setup` 配置供应商 API key，然后运行 `orca run \"你的任务\"`。",
	...
	SkipStaleCustomEntryFmt:    "跳过 orca.toml 里的旧 %q 条目（指向 %s）— 请手动从 [[providers]] 里删除",
	...
	UsageRunHint:              "用法：orca run [--model NAME] <task>",
	ErrorPrefix:               "错误：",
	...
	ProviderErrAuth:                "认证失败 (HTTP 401)：供应商 API key 缺失、错误或已过期。请检查供应商设置，或运行 `orca setup`。",
	...
	UsageBody: `orca — 由配置和插件驱动的 coding agent（多模型）

用法：
  orca chat [--model NAME] [-c|--continue] [--resume]   交互式会话（多轮；-c 恢复最近一次，--resume 选择一个）
  orca run  [--model NAME] [--max-steps N] [-c|--continue] [--resume PATH] <task>   执行单次任务后退出
  orca serve [--model NAME] [--addr HOST:PORT]      通过 HTTP+SSE 提供会话（浏览器客户端在 /）
  orca acp [--model NAME]                           通过 stdio 提供 Agent Client Protocol（也可用：orca --acp）
  orca setup [path]                                 交互式配置向导；生成 orca.toml（及 .env）
  orca config auto-plan [off|on]                    配置自动计划模式
  orca mcp <add|remove|list>                        管理 orca.toml 里的 MCP 服务器
  orca doctor [--json]                              输出脱敏的本地诊断信息
  orca bot start|doctor|weixin-login                多渠道 IM bot 网关
  orca version
  orca help
...
示例：
  orca chat
  orca chat --continue
  orca run "把 main.go 里的 TODO 实现掉"
  orca run --model mimo-pro "给这个函数补单元测试"
  echo "解释这段代码" | orca run

配置：
  优先级：flag > ./orca.toml > ~/.config/orca/config.toml > 内置默认值
  密钥通过 api_key_env 从环境变量注入。
  运行 'orca setup' 生成配置；详见 docs/SPEC.md。
`,
}
