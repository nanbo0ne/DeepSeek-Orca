...
var Chinese = Messages{
	...
	SetKeyHint:      "设置好 API key 后运行 `deepcode chat`。",
	ConfigLabel:     "配置",
	ModelsLabel:     "模型",
	...
	Ready:           "已就绪",
	GetStarted:      "开始使用",
	StepScaffold:    "生成 deepcode.toml",
	StepSetKey:      "设置 API key",

	InitHint:       "项目记忆（AGENTS.md）在会话内由模型生成：运行 `deepcode chat`，然后 `/init` —— 模型会分析代码库并写入。配置请用 `deepcode setup`。",
	StepSetKeyHint: "运行 `deepcode setup`，或 export DEEPSEEK_API_KEY=…",
	StepChatDesc:   "交互式会话",
	StepRunDesc:    "执行单次任务",
	HelpFooter:     "deepcode help · 查看全部命令",

	ChatTip:           "对话上下文将跨轮保留。输入 'exit' 或按 Ctrl-D 退出。",
	TurnCancelled:     "已取消 — 回到提示符",
	NoSessionToResume: "没有可恢复的会话 — 用 `deepcode chat` 开一个新的",
	...
	OutputStyleHint:    "在 deepcode.toml 设置 agent.output_style 即可启用（下次会话生效）",
	ThemeHeader:        "主题：",
	...
	SlashMCPNone:                 "没有配置 MCP 服务器 — 在 deepcode.toml 加一个 [[plugins]] 条目",
	...
	ArgEffortMax:    "最高推理",
	ArgThemeCurrent: "当前",
	ArgLanguageAuto: "从 DEEPCODE_LANG / 系统 locale 自动检测",
	...
	ListHooksNone:       "无生效 hooks — 在 .deepcode/settings.json（项目，需信任后）或 ~/.deepcode/settings.json（全局）配置",
	ListMcpHeader:       "MCP 服务器",
	ListMcpNone:         "未连接 MCP 服务器 — 在 deepcode.toml（[[plugins]]）或项目 .mcp.json 中添加",

	MemoryNone:             "还没有加载任何记忆 — 输入 “/remember 内容” 可快速记录，也可以在项目根目录创建 DEEPCODE.md",
	...
	MissingKeyIntro:       "deepcode.toml 已配置好 — 只差一个 API key 就可以开始。",
	WroteFileFmt:          "已写入 %s",
	SetupComplete:         "设置完成。",
	SetupCancelled:        "设置已取消。",
	TryHintFmt:            "试试: %s",
	NextHint:              "下一步：设置 API key（运行 `deepcode setup` 或 export DEEPSEEK_API_KEY=...），然后运行 `deepcode run \"你的任务\"`。",
	...
	SkipStaleCustomEntryFmt:    "跳过 deepcode.toml 里的旧 %q 条目（指向 %s）— 请手动从 [[providers]] 里删除",
	...
	UsageRunHint:              "用法：deepcode run [--model NAME] <task>",
	ErrorPrefix:               "错误：",
	...
	ProviderErrAuth:                "认证失败 (HTTP 401)：API key 缺失、错误或已过期。请检查 .env 中的密钥，或运行 `deepcode setup`。",
	...
	UsageBody: `deepcode — 由配置和插件驱动的 coding agent（多模型）

用法：
  deepcode chat [--model NAME] [-c|--continue] [--resume]   交互式会话（多轮；-c 恢复最近一次，--resume 选择一个）
  deepcode run  [--model NAME] [--max-steps N] [-c|--continue] [--resume PATH] <task>   执行单次任务后退出
  deepcode serve [--model NAME] [--addr HOST:PORT]      通过 HTTP+SSE 提供会话（浏览器客户端在 /）
  deepcode acp [--model NAME]                           通过 stdio 提供 Agent Client Protocol（也可用：deepcode --acp）
  deepcode setup [path]                                 交互式配置向导；生成 deepcode.toml（及 .env）
  deepcode config auto-plan [off|on]                    配置自动计划模式
  deepcode mcp <add|remove|list>                        管理 deepcode.toml 里的 MCP 服务器
  deepcode doctor [--json]                              输出脱敏的本地诊断信息
  deepcode bot start|doctor|weixin-login                多渠道 IM bot 网关
  deepcode version
  deepcode help
...
配置：
  ...
  运行 'deepcode setup' 生成配置；详见 docs/SPEC.md。
`,
}
