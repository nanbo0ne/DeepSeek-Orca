...
type Messages struct {
	...
	StepScaffold    string // step 1 desc — orca setup
	StepSetKey      string // step 2 command label

	// `orca init` — points to the in-session /init skill + setup
	InitHint       string
	StepSetKeyHint string // step 2 desc — env var hint
	StepChatDesc   string // orca chat step desc
	StepRunDesc    string // orca run step desc
	HelpFooter     string // dim footer linking to orca help
	...
	WroteFileFmt          string // "Wrote %s" — used for orca.toml and .env both
	...
	SkipStaleCustomEntryFmt    string // "skipping stale %q entry from orca.toml (pointing at %s) — please remove it"
	...
	UsageRunHint              string // "usage: orca run [--model NAME] <task>"
...
//
// Priority: override > ORCA_LANG > legacy V2 override > locale variables > "en".
...
func envCandidates() []string {
	keys := []string{"ORCA_LANG", "DEEPSEEK_ORCA_LANG", "LC_ALL", "LC_MESSAGES", "LANG"}
