...
// mcp.go holds the MCP server-management surface shared by the `deepcode mcp`
...
// mcpCommand implements `deepcode mcp <add|remove|list>`. It edits config only
...
func mcpList() int {
	...
	if bin, ok := codegraph.Resolve(cfg.Codegraph.Path); ok {
		...
		fmt.Printf("%-16s (built-in, not installed)%s  run `deepcode codegraph install`", "codegraph", codegraphMeta)
...
func mcpRemoveCLI(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: deepcode mcp remove <name>")
		return 2
	}
	name := args[0]
	cfg, err := config.Load()
...
func mcpUsage() {
	fmt.Println(`Manage MCP servers (persisted to deepcode.toml).

Usage:
  deepcode mcp list
  deepcode mcp add <name> <command> [args...]        stdio server
  deepcode mcp add <name> --http <url> [--header K=V] remote (Streamable HTTP)
  deepcode mcp add <name> --sse  <url>               remote (legacy SSE)
  deepcode mcp import                                import Codex-enabled servers from cc-switch
  deepcode mcp remove <name>
...
Examples:
  deepcode mcp add fs npx -y @modelcontextprotocol/server-filesystem .
  deepcode mcp add stripe --http https://mcp.stripe.com --header "Authorization=Bearer $STRIPE_KEY"
