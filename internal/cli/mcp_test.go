...
func TestMCPEditConfigLaunchUsesVisualBeforeEditor(t *testing.T) {
	...
	path := "/tmp/deepseek-orca config.toml"
...
func TestMCPEditConfigLaunchFallsBackToTerminalEditor(t *testing.T) {
	...
	launch, err := mcpEditConfigLaunchCommand("/tmp/deepseek-orca.toml", func(name string) (string, error) {
		if name == "vim" {
	...
	if len(launch.cmd.Args) != 2 || launch.cmd.Args[0] != "/usr/bin/vim" || launch.cmd.Args[1] != "/tmp/deepseek-orca.toml" {
...
func TestApplyMCPModeDropsLegacyTier(t *testing.T) {
	...
	if err := cfg.SaveTo("deepseek-orca.toml"); err != nil {
	...
	m.mcp = &mcpManager{
		stage: mcpStageMode,
		name:  "github",
		snapshot: mcpSnapshot{configPath: "deepseek-orca.toml", servers: []mcpServerView{{
	...
	raw, err := os.ReadFile("deepseek-orca.toml")
	if err != nil {
...
func TestApplyMCPModeRecordsPluginConnectFailure(t *testing.T) {
	...
	t.Setenv("PATH", "")
	cfg := config.Default()
	cfg.Plugins = []config.PluginEntry{{Name: "broken", Command: "definitely-missing-deepseek-orca-mcp", Tier: "lazy"}}
	if err := cfg.SaveTo("deepseek-orca.toml"); err != nil {
		t.Fatalf("save config: %v", err)
	}
