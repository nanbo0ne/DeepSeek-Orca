// Package cli implements deepcode's command-line entry: subcommand routing, flag
...
func Run(args []string, version string) int {
	...
	case "init":
		...
		// for config), so `deepcode init` isn't a dead end.
		configureCLIThemeFromConfigNoProbe()
		return initHint()
	case "acp":
		configureCLIThemeFromConfigNoProbe()
...
func runServe(args []string) int {
	...
	fmt.Printf("deepcode serve — %s on http://%s\n", ctrl.Label(), *addr)
...
// Keys always go to the deepcode-owned global credentials file so they never land
...
// project-local deepcode.toml only when the user config dir can't be resolved.
...
// defaultEnvTarget is the deepcode-owned global credentials file, falling back to
...
// resolveSetupTargets picks where `deepcode setup` writes. Keys always go to the
// global env. The config goes to the user-global dir by default, to ./deepcode.toml
...
// setupConfig runs the configuration wizard (the `deepcode setup` command),
// writing config.toml to the user-global dir (or ./deepcode.toml under --local)
// and API keys to the deepcode-owned global .env — never a project's own .env.
...
// initHint handles `deepcode init`. Unlike a config scaffold, project memory is
...
// (and to `deepcode setup` for config) so the verb isn't a dead end.
func initHint() int {
...
// and any entered API keys to envPath (the deepcode-owned global .env, never a
...
func interactiveSetup(configPath, envPath string) int {
	...
	fmt.Print(boxed([]string{
		accent("◆") + " " + fmt.Sprintf(i18n.M.WelcomeTitleFmt, bold("deepcode")),
		"",
...
// that older versions of the wizard wrote into deepcode.toml. They collide
...
// can warn the user to clean up deepcode.toml by hand.
...
// duplicate provider entries on subsequent re-runs of `deepcode setup`.
// The host-based slug also gives users a meaningful name to grep for in
// deepcode.toml. Falls back to a short sha1 of the raw URL when the URL
...
// not exist in config because users editing deepcode.toml deal with SKU names
...
// deepcode.toml that defines only [[providers]] for deepseek otherwise hides
...
// deepcode-owned global .env so the chat session that follows picks them up via
...
// are appended — so re-running `deepcode setup` with a corrected key replaces the
...
func welcome(version string) int {
	...
	b.WriteString(boxed([]string{
		accent("◆") + " " + bold("deepcode") + "  " + dim(version),
		dim(i18n.M.Subtitle),
	}))
	...
	if src == "" {
		step("deepcode setup", i18n.M.StepScaffold)
	}
	if ready == 0 {
		step(i18n.M.StepSetKey, i18n.M.StepSetKeyHint)
	}
	step("deepcode chat", i18n.M.StepChatDesc)
	step(`deepcode run "task"`, i18n.M.StepRunDesc)
...
func configAutoPlanCommand(args []string) int {
	...
	local := fs.Bool("local", false, "write ./deepcode.toml instead of the user config")
	if err := fs.Parse(args); err != nil {
		return 2
	}
	rest := fs.Args()
...
func configUsage() {
	fmt.Print(`Usage:
  deepcode config auto-plan [--local] [off|on]
`)
}
