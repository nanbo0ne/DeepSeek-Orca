...
func TestGitStatusRenderWithinCompactsRepoBeforeBranch(t *testing.T) {
	status := gitStatus{Repo: "VeryLongDeepSeekDeepCodeWorkspace", Branch: "codex/cli-tui-status-row"}

	full := ansi.Strip(status.RenderWithin(80, statusAutoColor))
	if full != "VeryLongDeepSeekDeepCodeWorkspace@codex/cli-tui-status-row" {
...
func TestGitStatusRenderWithinKeepsDirtySuffix(t *testing.T) {
	status := gitStatus{
		Repo:      "VeryLongDeepSeekDeepCodeWorkspace",
...
func TestLoadGitStatus(t *testing.T) {
	...
	runGitForTest(t, root, "config", "user.email", "deepcode@example.invalid")
	runGitForTest(t, root, "config", "user.name", "DeepCode Test")
