...
command = "local-bin"
`
	if err := os.WriteFile("deepcode.toml", []byte(toml), 0o644); err != nil {
		t.Fatal(err)
	}
	mcp := `{ "mcpServers": {
  ...
  "extra":  { "command": "extra-bin", "auto_start": false }
	...
	if byName["shared"].Command != "local-bin" || byName["shared"].URL != "" {
		t.Errorf("deepcode.toml should win the collision, got %+v", byName["shared"])
	}
...
func TestLoadMergesPluginsAcrossTOMLSources(t *testing.T) {
	...
	if err := os.WriteFile("deepcode.toml", []byte("[[plugins]]\nname = \"projectmcp\"\ncommand = \"project-bin\"\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if !names["globalmcp"] || !names["projectmcp"] {
		t.Fatalf("a project deepcode.toml [[plugins]] dropped the global config's server; got %+v", cfg.Plugins)
	}
}
...
func TestMergeMCPJSONPrecedence(t *testing.T) {
	// deepcode.toml already declares "shared" (stdio); .mcp.json offers a colliding
	// "shared" (http) plus a fresh "extra". deepcode.toml must win on the collision;
	// "extra" gets appended.
	...
	if cfg.Plugins[0].Name != "shared" || cfg.Plugins[0].Command != "local-bin" || cfg.Plugins[0].URL != "" {
		t.Errorf("collision not won by deepcode.toml: %+v", cfg.Plugins[0])
	}
...
name = "dida"
type = "http"
url = "https://deepcode.example/mcp?access_token=toml"
[plugins.headers]
...
`), 0o644); err != nil {
  ...
  "dida": {
	...
	if source != "deepcode.toml" {
		t.Fatalf("source = %q, want deepcode.toml", source)
	}
	if updated.URL != "https://deepcode.example/mcp" {
		t.Fatalf("updated URL = %q", updated.URL)
	}

	projectRaw, err := os.ReadFile("deepcode.toml")
	if err != nil {
		t.Fatal(err)
	}
	if strings.Contains(string(projectRaw), "access_token=toml") || strings.Contains(string(projectRaw), "Authorization") {
		t.Fatalf("deepcode.toml auth material should be removed:\n%s", projectRaw)
	}
