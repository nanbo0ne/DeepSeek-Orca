...
func TestRenderTOMLRoundTrips(t *testing.T) {
	...
	orig.Network = NetworkConfig{
		...
		Proxy: NetworkProxyConfig{
			...
			Port:     7890,
			Username: "user",
			Password: "${DEEPSEEK_ORCA_PROXY_PASSWORD}",
		},
	}
	...
	if got.ConfigVersion != 10 {
		t.Errorf("config_version = %d, want 10", got.ConfigVersion)
	}
	if got.Language != "zh" {
...
func TestScopedRenderSeparatesUserAndProjectConfig(t *testing.T) {
	...
	for _, want := range []string{"config_version = 10", "[desktop]", `theme = "light"`, `close_behavior = "background"`, `check_updates = false`, "[notifications]"} {
