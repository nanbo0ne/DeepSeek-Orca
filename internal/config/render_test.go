...
func TestRenderTOMLRoundTrips(t *testing.T) {
	...
	if got.ConfigVersion != 7 {
		t.Errorf("config_version = %d, want 7", got.ConfigVersion)
	}
	if got.Language != "zh" {
...
func TestScopedRenderSeparatesUserAndProjectConfig(t *testing.T) {
	...
	for _, want := range []string{"config_version = 7", "[desktop]", `theme = "light"`, `close_behavior = "background"`, `check_updates = false`, "[notifications]"} {
