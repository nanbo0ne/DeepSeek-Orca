...
func TestRenderTOMLRoundTrips(t *testing.T) {
	...
	if got.ConfigVersion != 9 {
		t.Errorf("config_version = %d, want 9", got.ConfigVersion)
	}
	if got.Language != "zh" {
...
func TestScopedRenderSeparatesUserAndProjectConfig(t *testing.T) {
	...
	for _, want := range []string{"config_version = 9", "[desktop]", `theme = "light"`, `close_behavior = "background"`, `check_updates = false`, "[notifications]"} {
