...
// RenderTOML renders the config as annotated TOML in the `deepcode setup` house style:
...
// deepcode.toml stays focused on project behavior and intentionally excludes
...
func RenderTOMLForScope(c *Config, scope RenderScope) string {
	...
	b.WriteString("# DeepCode configuration.\n")
	b.WriteString("# Resolution order: flag > ./deepcode.toml > ~/.config/deepcode/config.toml > built-in defaults.\n")
	...
	if c.Language != "" {
		fmt.Fprintf(&b, "language      = %q   # ui/model language; empty = auto-detect from $LANG / $DEEPCODE_LANG\n", c.Language)
	} else {
		b.WriteString("# language      = \"zh\"   # ui/model language; empty = auto-detect from $LANG / $DEEPCODE_LANG\n")
	}
	b.WriteString("\n")

	if shouldRenderUI(c, defaults, scope) {
		b.WriteString("[ui]\n")
		fmt.Fprintf(&b, "theme = %q   # auto|dark|light; CLI colors only; DEEPCODE_THEME can override per run\n", c.UITheme())
		if style := c.UIThemeStyle(); style != "" {
			fmt.Fprintf(&b, "theme_style = %q   # CLI accent palette; DEEPCODE_THEME_STYLE can override per run\n", style)
		} else {
	...
	if shouldRenderNetwork(c, defaults, scope) {
		...
		if c.Network.Proxy.Password != "" {
			...
			b.WriteString("# password = \"${DEEPCODE_PROXY_PASSWORD}\"   # optional; supports ${VAR} expansion\n")
		}
		b.WriteString("\n")
	}
	...
	if c.Codegraph.Path != "" {
		...
		b.WriteString("# path       = \"\"   # empty = cache, then PATH, then a bundle beside deepcode\n")
	}
	b.WriteString("\n")
	...
	if len(c.Plugins) == 0 {
		...
		b.WriteString("# command = \"deepcode-plugin-example\"\n")
...
// active TOML array; an empty one renders as a commented example so `deepcode setup`
