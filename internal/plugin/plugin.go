// Package plugin is O.R.C.A's MCP client. It connects to external MCP servers and
...
// protocolVersion is the MCP revision O.R.C.A advertises during initialize.
...
type Spec struct {
	...
	// inherit orca's cwd (the default for user-configured plugins). It exists
...
// messages (notifications, requests like roots/list) are ignored — O.R.C.A is a
...
func (c *Client) initialize(ctx context.Context) error {
	res, err := c.call(ctx, "initialize", map[string]any{
		...
		"clientInfo":      map[string]any{"name": "orca", "version": "dev"},
	})
	if err != nil {
		return err
	}
...
type mcpTool struct {
	...
	// plugin that declares a tool read-only opts it into O.R.C.A's parallel-dispatch
