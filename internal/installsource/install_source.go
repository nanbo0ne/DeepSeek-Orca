...
func (t *installSourceTool) Execute(ctx context.Context, raw json.RawMessage) (string, error) {
	...
	if len(actions) == 0 {
		out := response{
			...
			PlanID:   planID,
			Warnings: warnings,
			Next:     "No installable Orca skill or MCP server was detected. Ask the user for a direct SKILL.md, skill root, .mcp.json, MCP endpoint, or package name.",
		}
...
func (t *installSourceTool) resolveSkillPath(name, scope string) (string, bool) {
	...
	if scope == "global" {
		...
		roots = []string{
			filepath.Join(t.home, product.ProjectStateDir, skill.SkillsDirname),
			filepath.Join(t.home, product.LegacyProjectStateDir, skill.SkillsDirname),
		}
	} else {
		roots = []string{
			filepath.Join(t.root, product.ProjectStateDir, skill.SkillsDirname),
			filepath.Join(t.root, product.LegacyProjectStateDir, skill.SkillsDirname),
		}
	}
...
func (t *installSourceTool) configPath(scope string) string {
	...
	return filepath.Join(t.root, product.ProjectConfigName)
}
