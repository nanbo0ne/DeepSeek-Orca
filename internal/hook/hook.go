...
// (.deepcode/settings.json, only when the project is trusted) and a global
// (~/.deepcode/settings.json) file. A hook's exit
...
const (
	SettingsDirname  = ".deepcode"
	SettingsFilename = "settings.json"
)

// GlobalSettingsPath is ~/.deepcode/settings.json (homeDir overrides ~).
func GlobalSettingsPath(homeDir string) string {
	return filepath.Join(home(homeDir), SettingsDirname, SettingsFilename)
}

// ProjectSettingsPath is <root>/.deepcode/settings.json.
