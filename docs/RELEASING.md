...
How DeepCode ships, who can ship what, and the canary-before-stable flow.
...
|---|---|---|
| npm | `latest` (0.x), `next` (1.x) | `canary` (`npm i deepcode@canary`) |
...
3. **Test** — testers install `deepcode@canary` (CLI) or grab the desktop canary
...
6. **Promote to default install** (optional, when 1.x should become the bare `npm i` target):
   ```sh
   npm dist-tag add deepcode@1.4.0 latest
   ```
