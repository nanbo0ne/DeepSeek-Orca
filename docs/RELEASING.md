...
|---|---|---|
| npm | `latest` (0.x), `next` (1.x) | `canary` (`npm i deepseek-orca@canary`) |
...
3. **Test** — testers install `deepseek-orca@canary` (CLI) or grab the desktop canary
...
6. **Promote to default install** (optional, when 1.x should become the bare `npm i` target):
   ```sh
   npm dist-tag add deepseek-orca@1.4.0 latest
   ```
