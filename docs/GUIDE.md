...
Resolution order: **flag > `./deepcode.toml` > `~/.config/deepcode/config.toml` >
...
# language    = "zh"               # ui language; empty = auto-detect from $LANG / $DEEPCODE_LANG
...
`deepcode chat` can grant Bash as an exact command or as a conservative command
...
`deepcode run` stays autonomous but still honours `deny`.
...
DeepCode is an MCP client. A `[[plugins]]` entry's `type` selects the transport:
...
exposes. `make build` also produces `bin/deepcode-plugin-example` — a runnable
...
**Already have an `.mcp.json`?** Drop it in the project root and DeepCode
reads it as-is — the `mcpServers` spec (`command`/`args`/`env`, `type`/`url`/
`headers`, `${VAR}` expansion) maps field-for-field onto `[[plugins]]`. Both
sources are merged; on a name collision `deepcode.toml` wins.
...
**Upgrading from `0.x`?** Your old `~/.deepcode/config.json` is still read for its
`mcpServers` (honouring `mcpDisabled`) as a lowest-priority source, so MCP servers
keep working — move them into `deepcode.toml`'s `[[plugins]]` or a `.mcp.json` when
convenient.

## Slash commands

In `deepcode chat`, built-in commands (`/compact`, `/new`, `/clear`, `/rewind`,
...
another branch. **Custom commands** are Markdown files under `.deepcode/commands/`
(project) or `~/.config/deepcode/commands/` (user) — `review.md` becomes
...
Embed `@` references in a message and DeepCode resolves them before sending, as
...
`deepcode setup` keeps first-run minimal: pick provider → keys (every SKU of a
...
The planner sees loaded `DEEPCODE.md` / `AGENTS.md` memory and a small read-only
...
`./deepcode.toml` only when that repository needs a shared override, such as a
...
automatically: DeepCode first drafts a read-only plan, then waits for approval
...
`/auto-plan off|on` in `deepcode chat` to change the user-level setting, or
`deepcode config auto-plan off|on` from a shell/script. Pass `--local` to the
