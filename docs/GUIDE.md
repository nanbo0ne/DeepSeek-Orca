...
Resolution order: **flag > `./deepseek-orca.toml` > `~/.config/deepseek-orca/config.toml` >
...
# language    = "zh"               # ui language; empty = auto-detect from $LANG / $DEEPSEEK_ORCA_LANG
...
`deepseek-orca chat` can grant Bash as an exact command or as a conservative command
...
`deepseek-orca run` stays autonomous but still honours `deny`.
...
DeepSeek-Orca is an MCP client. A `[[plugins]]` entry's `type` selects the transport:
...
exposes. `make build` also produces `bin/deepseek-orca-plugin-example` — a runnable
...
**Already have an `.mcp.json`?** Drop it in the project root and DeepSeek-Orca
reads it as-is — the `mcpServers` spec (`command`/`args`/`env`, `type`/`url`/
`headers`, `${VAR}` expansion) maps field-for-field onto `[[plugins]]`. Both
sources are merged; on a name collision `deepseek-orca.toml` wins.
...
**Upgrading from `0.x`?** Your old `~/.deepseek-orca/config.json` is still read for its
`mcpServers` (honouring `mcpDisabled`) as a lowest-priority source, so MCP servers
keep working — move them into `deepseek-orca.toml`'s `[[plugins]]` or a `.mcp.json` when
convenient.

## Slash commands

In `deepseek-orca chat`, built-in commands (`/compact`, `/new`, `/clear`, `/rewind`,
...
another branch. **Custom commands** are Markdown files under `.deepseek-orca/commands/`
(project) or `~/.config/deepseek-orca/commands/` (user) — `review.md` becomes
...
Embed `@` references in a message and DeepSeek-Orca resolves them before sending, as
...
`deepseek-orca setup` keeps first-run minimal: pick provider → keys (every SKU of a
...
The planner sees loaded `DEEPSEEK_ORCA.md` / `AGENTS.md` memory and a small read-only
...
`./deepseek-orca.toml` only when that repository needs a shared override, such as a
...
automatically: DeepSeek-Orca first drafts a read-only plan, then waits for approval
...
`/auto-plan off|on` in `deepseek-orca chat` to change the user-level setting, or
`deepseek-orca config auto-plan off|on` from a shell/script. Pass `--local` to the
