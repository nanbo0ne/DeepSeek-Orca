...
build:
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o bin/deepcode$(GOEXE) ./cmd/deepcode
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o bin/deepcode-plugin-example$(GOEXE) ./cmd/deepcode-plugin-example
...
cross:
	...
	@for p in darwin/amd64 darwin/arm64 linux/amd64 linux/arm64 windows/amd64 windows/arm64; do \
		...
		echo "build $$os/$$arch"; \
		CGO_ENABLED=0 GOOS=$$os GOARCH=$$arch go build -ldflags "$(LDFLAGS)" -o dist/deepcode-$$os-$$arch$$ext ./cmd/deepcode; \
	done
...
# layout: launcher at bin/codegraph/bin/codegraph beside bin/deepcode) and run the
# gated MCP end-to-end test against it. Requires `gh`. Windows: install via the
# upstream install.ps1 and run the test with DEEPCODE_CODEGRAPH_BIN set.
e2e-codegraph:
	...
	DEEPCODE_CODEGRAPH_E2E=1 DEEPCODE_CODEGRAPH_BIN=$$PWD/$$dest/bin/codegraph \
